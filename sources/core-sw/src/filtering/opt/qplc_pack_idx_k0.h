...
OWN_OPT_FUN(qplc_status_t, k0_qplc_pack_index_8u,
    ...
    if (act_num_elements > num_elements) act_num_elements = num_elements;
    ...
    *index_ptr += act_num_elements;
    if ((act_num_elements < num_elements) && (QPLC_STS_OK == status)) {
        ...
        if (QPLC_STS_OK == status) *index_ptr += num_elements + num_elem_rem;
    }
    *pp_dst = dst_ptr;
    return status;
}
...
OWN_OPT_FUN(qplc_status_t, k0_qplc_pack_index_8u32u,
    ...
    if (index) act_num_elements -= (index - 1);
    if (act_num_elements > num_elements) act_num_elements = num_elements;
