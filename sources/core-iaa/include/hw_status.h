...
#define AD_STATUS_WRITE_PAGE_FAULT \
    (3U + 128U) /**< Page Fault happened on Write (in this case 7th bit of status is set to 1) */
/** @} */
...
#define AD_ERROR_CODE_BIGHDR \
    1U /**< Reached the end of the input stream before decoding header and header is too big to fit in input buffer */
...
#define AD_ERROR_CODE_LEN_SPANS_MBLOCKS \
    27U /**< Indexing only: A match had a length expending into the next mini-block */
...
#define AD_ERROR_CODE_TOO_FEW_ELEMENTS_PROCESSED \
    32U /**< The input stream ended before specified Number of input Element was seen */
#define AD_ERROR_CODE_INVALID_ZDECOMP_HDR \
    34U /**< During a `Z-Decompress` operation, the input data ended in the middle of a record header */
