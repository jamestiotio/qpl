...
static inline void check_destination_overflow_prefetch(qpl_job* const job_ptr, qpl_operation operation,
                                                       uint32_t flags) {
...
QPL_LOW_LEVEL_API_BAD_ARGUMENT_TEST(scan, drop_initial_bytes) {
    ...
    check_drop_initial_bytes_verification<operation_group_e::filter_single_source>(job_ptr, operation, OPERATION_FLAGS);
