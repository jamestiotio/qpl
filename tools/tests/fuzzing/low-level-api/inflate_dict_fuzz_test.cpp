...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* Data, size_t Size) {
    ...
    if ((sizeof(inflate_properties) * 2) < Size) {
        ...
        if (properties->dictionary_size & 4096) { compression_level = qpl_high_level; }
        if (0 == destination_size) { destination_size = 1; }
        if (0 == dictionary_size) { dictionary_size = 1; }
...

    {
        ...
        qpl_status status =
                qpl_build_dictionary(dictionary_ptr, sw_level, HW_NONE, dictionary_data_ptr, dictionary_size);
