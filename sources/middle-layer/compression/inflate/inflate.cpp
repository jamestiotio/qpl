...
static auto inflate_pass(isal_inflate_state& inflate_state, uint8_t* output_start_ptr) noexcept -> qpl_ml_status;

static auto own_inflate(inflate_state<execution_path_t::software>& decompression_state,
                        end_processing_condition_t                 end_processing_condition) noexcept
        -> decompression_operation_result_t;

static auto own_inflate_random(inflate_state<execution_path_t::software>& decompression_state) noexcept
        -> decompression_operation_result_t;

template <execution_path_t path, inflate_mode_t mode>
auto inflate(inflate_state<path>& decompression_state, end_processing_condition_t end_processing_condition) noexcept
        -> decompression_operation_result_t {
    ...
    if (result.status_code_ == status_list::ok) {
        if (decompression_state.is_final() &&
            (decompression_state.get_state()->tmp_out_processed != decompression_state.get_state()->tmp_out_valid)) {
...
template auto inflate<execution_path_t::software, inflate_mode_t::inflate_default>(
...
template auto inflate<execution_path_t::software, inflate_mode_t::inflate_header>(
        inflate_state<execution_path_t::software>& decompression_state,
        end_processing_condition_t end_processing_condition) noexcept -> decompression_operation_result_t;

template auto inflate<execution_path_t::software, inflate_mode_t::inflate_body>(
        inflate_state<execution_path_t::software>& decompression_state,
        end_processing_condition_t end_processing_condition) noexcept -> decompression_operation_result_t;
...
static auto own_inflate(inflate_state<execution_path_t::software>& decompression_state,
    ...
    if (is_internal_buffer_available) {
        ...
        while (do_next_inflate_pass) {
            ...
            if (status_list::ok != result.status_code_) { break; }

            // Check if inflate should be stopped
            do_next_inflate_pass = !utility::is_inflate_complete(end_processing_condition, *inflate_state_ptr);
        }
        inflate_state_ptr->tmp_out_valid =
                static_cast<int32_t>(inflate_state_ptr->next_out - inflate_state_ptr->tmp_out_buffer);
    ...
    if (inflate_state_ptr->tmp_out_processed == inflate_state_ptr->tmp_out_valid) {
        ...
        bool do_next_inflate_pass = !utility::is_inflate_complete(end_processing_condition, *inflate_state_ptr);
        ...
        if (!is_internal_buffer_available && inflate_state_ptr->block_state == ISAL_BLOCK_NEW_HDR) {
    ...
    const uint32_t bytes_in_internal_buffer = inflate_state_ptr->copy_overflow_length +
                                              inflate_state_ptr->write_overflow_len + inflate_state_ptr->tmp_out_valid;

    const bool is_internal_buffer_overflowed = bytes_in_internal_buffer > sizeof(inflate_state_ptr->tmp_out_buffer);

    if (ISAL_BLOCK_INPUT_DONE != inflate_state_ptr->block_state || is_internal_buffer_overflowed) {
        ...
        if (inflate_state_ptr->tmp_out_valid == inflate_state_ptr->tmp_out_processed
            ...
            core_sw::util::copy(inflate_state_ptr->next_out - ISAL_DEF_HIST_SIZE, inflate_state_ptr->next_out,
            ...
            memmove(inflate_state_ptr->tmp_out_buffer, &inflate_state_ptr->tmp_out_buffer[shift_size],
...
auto own_inflate_random(inflate_state<execution_path_t::software>& decompression_state) noexcept
        -> decompression_operation_result_t {
    auto inflate_state_ptr = decompression_state.get_state();
    ...
    inflate_state_ptr->block_state =
            ISAL_BLOCK_CODED; // todo: Incorrect behavior in case if block is actually stored, fix
...
static auto inflate_pass(isal_inflate_state& inflate_state, uint8_t* output_start_ptr) noexcept -> qpl_ml_status {
    ...
    if (ISAL_BLOCK_NEW_HDR == inflate_state.block_state || ISAL_BLOCK_HDR == inflate_state.block_state) {
...
static inline auto own_inflate(inflate_state<execution_path_t::hardware>& decompression_state) {
    ...
    auto result = util::process_descriptor<decompression_operation_result_t, util::execution_mode_t::sync>(
            descriptor, completion_record);

    if (result.status_code_ == status_list::ok) { result.completed_bytes_ = decompression_state.get_input_size(); }
...
auto inflate<execution_path_t::hardware, inflate_mode_t::inflate_header>(
        inflate_state<execution_path_t::hardware>& decompression_state,
        end_processing_condition_t                 UNREFERENCED_PARAMETER(end_processing_condition)) noexcept
        -> decompression_operation_result_t {
    ...
    if (ISAL_BLOCK_FINISH == inflate_state.block_state) {
        ...
        if (inflate_state.tmp_out_valid < 2 * ISAL_DEF_HIST_SIZE) {
            ...
            inflate_state.avail_out =
                    sizeof(inflate_state.tmp_out_buffer) - ISAL_LOOK_AHEAD - inflate_state.tmp_out_valid;

            if ((int32_t)inflate_state.avail_out < 0) { inflate_state.avail_out = 0U; }
        } else {
            is_state_decoding_into_internal_buffer = false;
        }
    }
...
static void handle_internal_buffers_overflow(isal_inflate_state& inflate_state) noexcept {
    ...
    if (0U != inflate_state.copy_overflow_length) {
        auto lookback_ptr =
                (&inflate_state.tmp_out_buffer[inflate_state.tmp_out_valid]) - inflate_state.copy_overflow_distance;
