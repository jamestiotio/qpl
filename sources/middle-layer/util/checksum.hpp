...
/**
 * @brief Lookup table for crc32 gzip. Copied from sources/isal/crc/crc_base.c
 */
const uint32_t crc32_table_gzip_refl[256] = {
...
inline uint32_t crc32_gzip(const input_iterator_t source_begin, const input_iterator_t source_end,
    ...
    return qpl_crc32_gzip_refl(seed, reinterpret_cast<const uint8_t*>(&*source_begin),
                               length * sizeof(typename std::iterator_traits<input_iterator_t>::value_type));
}

template <class input_iterator_t>
inline uint32_t crc32_iscsi_inv(const input_iterator_t source_begin, const input_iterator_t source_end,
                                uint32_t seed) noexcept {
    ...
    return ~qpl_crc32_iscsi(reinterpret_cast<const uint8_t*>(&*source_begin),
                            length * sizeof(typename std::iterator_traits<input_iterator_t>::value_type), ~seed);
}

template <class input_iterator_t>
inline uint32_t xor_checksum(const input_iterator_t source_begin, const input_iterator_t source_end,
                             uint32_t seed) noexcept {
    // Check if we've got valid iterators
    static_assert(std::is_same<typename std::iterator_traits<input_iterator_t>::iterator_category,
                               std::random_access_iterator_tag>::value,
